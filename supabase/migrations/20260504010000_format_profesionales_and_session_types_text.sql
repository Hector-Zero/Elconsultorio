-- ─────────────────────────────────────────────────────────────────────────────
-- Migration: format profesionales and session_types as readable Spanish text
-- ─────────────────────────────────────────────────────────────────────────────
-- Adds two new top-level fields to get_bot_context:
--   - profesionales_text: compact text version of professionals (name, services
--     with prices, specialties)
--   - session_types_text: compact text version of services (name and price)
--
-- The original profesionales array is kept (used by dashboard/public URL).
-- The original session_types array is also kept.
--
-- Format example:
--   profesionales_text:
--     === Profesional 1 ===
--     Servicios:
--       - Consulta individual: $50.000 CLP
--       - Consulta de pareja: $60.000 CLP
--       - Consulta familiar: $80.000 CLP
...
BEGIN
  ...
  WITH pro_services AS (
    ...
    GROUP BY p.id, p.full_name, p.created_at, p.specialties
  )
  SELECT string_agg(
    '=== ' || profesional || ' ===' || E'\n' ||
    'Servicios:' || E'\n' ||
    ...
    CASE
      WHEN specialties IS NOT NULL AND array_length(specialties, 1) > 0
      THEN E'\nEspecialidades: ' || array_to_string(specialties, ', ')
      ELSE ''
    END,
    E'\n\n'
  ...
  -- (unchanged from previous migration)
  WITH days AS (
  ...
  SELECT jsonb_build_object(
    ...
    -- Original session_types array kept for downstream use
    ...
    -- NEW: compact text for the bot
    'session_types_text', to_jsonb(v_session_types_text),
    -- Original profesionales array kept (dashboard, public URL)
    'profesionales', (
      SELECT jsonb_agg(
        jsonb_build_object(
          'nombre',           p.full_name,
    ...
    -- Pre-formatted slots for the bot
